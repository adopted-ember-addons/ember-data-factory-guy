...
test("#clearStore clears the store of models, and resets the model definition", function() {
  Ember.run(function() {
    ...
    equal(store.peekAll('user').get('content.length'), 0);
    equal(store.peekAll('project').get('content.length'), 0);
...
test("creates list of DS.Model instances", function() {
  ...
  var storeUsers = store.peekAll('user').get('content');
  equal(storeUsers.length , 2);
});
